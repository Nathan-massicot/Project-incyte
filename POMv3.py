...
def compute_schedule(df: pd.DataFrame, j0_datetime: datetime, reverse_to_j0: bool = True) -> pd.DataFrame:
    ...
    if {"Task_ID", "Predecessors"}.issubset(gantt_df.columns):
        ...
        # Anchor: if classic J0 mode, build relative then shift to J0; otherwise start at the custom start date directly
        project_start = j0_datetime if not reverse_to_j0 else (j0_datetime - timedelta(days=365))
